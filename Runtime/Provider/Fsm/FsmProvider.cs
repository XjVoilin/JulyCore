...
{
    public class FsmProvider : ProviderBase, IFsmProvider
    {
        ...
        private sealed class FsmInstance : IFsm
        {
            ...
            public bool ChangeState(int newState)
            {
                ...
                if (!_states.TryGetValue(newState, out var newInstance))
                    return false;

                if (_currentInstance != null && !_currentInstance.CanChangeTo(newState))
                    return false;
